# FROM PY_ECC APART FROM ecdsa_unsafe_sign AND ecdsa_raw_verify
...
# CUSTOM FUNCTIONALITY I ADDED FOR USE
def ecdsa_unsafe_sign(msghash: bytes, nonce: int, priv: bytes) -> Tuple[int, int, int]:
    z = bytes_to_int(msghash)
    k = nonce
...
def ecdsa_raw_verify(
    msghash: bytes, vrs: Tuple[int, int, int], pubKey: "PlainPoint2D"
) -> bool:
    v, r, s = vrs
    z = bytes_to_int(msghash)
    sInv = pow(s, -1, N)

    a = jacobian_multiply(cast("PlainPoint3D", (Gx, Gy, 1)), (z * sInv))
    b = jacobian_multiply(cast("PlainPoint3D", (pubKey[0], pubKey[1], 1)), (r * sInv))

    c = from_jacobian(jacobian_add(a, b))
